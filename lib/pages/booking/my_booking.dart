...
class _MyBookingState extends State<MyBooking> {
  ...
  upcomingBookings() {
    return FutureBuilder(
      future: ApiProvider.getMethodAuthorized('/api/getbooking'),
      //  future: ApiProvider.getMethodAuthorized('/api/getafterdate'),
      builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
        if (snapshot.connectionState == ConnectionState.done &&
            snapshot.hasData) {
          dynamic upcomingBookingList = snapshot.data["getbooking"];
          // dynamic upcomingBookingList = snapshot.data["getafterdate"];
                ...
                onTap: () {
                  Navigator.push(
                    ...
                    PageTransition(
                      ...
                      child: UpcomingBooking(bookingData: data),
                    ),
                  );
                },
                child: Column(
  ...
  Future<dynamic> getPastBookings() async {
    dynamic data = await ApiProvider.getMethodAuthorized('/api/getbeforedate');
    for (int i = 0; i < data["getbeforedate"].length; i++) {
      dynamic id = data["getbeforedate"][i]["service"];
      dynamic servicemandata =
      await ApiProvider.getMethod('/api/getsingleservice/${id}');
  ...
  pastBookings() {
    ...
    return FutureBuilder(
      ...
      builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
        if (snapshot.connectionState == ConnectionState.done &&
            snapshot.hasData) {
          dynamic pastBookingList = snapshot.data["getbeforedate"];
          return ListView.builder(
